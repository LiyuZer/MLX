...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    """Parse CLI args for optional argv list; defaults to sys.argv[1:]."""
    parser = build_parser()
    return parser.parse_args(argv)


# Back-compat: Provide previous function name
# Note: kept for equivalence with the original API shape.
parse_arguments = parse_args  # type: ignore
...
def _resolve_message(args: argparse.Namespace, msg_attr: str, file_attr: str) -> Optional[str]:
...
def _print_hypothesis_details(h) -> None:
    print(f"Name: {h.name}")
    print(f"ID: {str(h.id)}")
...
def _list_all_experiments(core) -> list:
    items = []
    def walk(h):
...
def _print_experiment_details(exp) -> None:
